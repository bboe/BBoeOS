...
static int _utoa(unsigned int v, int base, int upper, int min_digits,
                 char *out) {
...
int vsnprintf(char *buf, size_t cap, const char *fmt, va_list ap) {
    ...
    while (*fmt) {
        ...
        case 'i': {
            ...
            if (v < 0) {
                prefix[0] = '-';
                body_len = _utoa((unsigned)(-v), 10, 0, prec, num);
            } else if (plus) {
                prefix[0] = '+';
                body_len = _utoa((unsigned)v, 10, 0, prec, num);
            } else
