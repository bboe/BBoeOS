#!/bin/sh
# tests/test_pmode.sh — build and run the standalone pmode smoke test.
#
...
nasm -f bin -i src/kernel/ -o tests/pmode_test.bin tests/pmode_test.asm
...
echo "Booting pmode smoke test (expecting 'R' then 'P' on serial)..."
