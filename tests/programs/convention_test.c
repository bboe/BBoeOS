 ...
 *
 *   register_one_arg — exercises the implicit register-passing default
 *                      for 1-arg callees: arg 0 in AX before call, callee
 *                      spills to a local slot.  Covers literal / local /
 *                      expression / nested-fastcall call sites.
 *                      Previously fctest.c.
 *
 *   register_three_arg — exercises the implicit default for 2- and 3-arg
 *                        callees: args 0/1/2 in EAX/EDX/ECX. */
...
int add_one(int v);
int blend(int hi, int lo);
int doubled(int v);
int fan_in(int a, int b, int c);
__attribute__((carry_return)) int is_positive(int v);
void mode_carry();
void mode_register_one_arg();
void mode_register_three_arg();
...
int main(int argc, char *argv[]) {
    ...
    if (string_equal(mode, "carry")) {
        mode_carry();
    } else if (string_equal(mode, "register_one_arg")) {
        mode_register_one_arg();
    } else if (string_equal(mode, "register_three_arg")) {
        mode_register_three_arg();
    } else {
...
void mode_register_three_arg() {
    ...
    /* 2-arg callee: arg 0 → EAX, arg 1 → EDX. */
    printf("blend(7,8)       = %d\n", blend(7, 8)); /* 708 */
