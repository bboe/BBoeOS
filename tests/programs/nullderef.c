/* Smoke test for the user-fault kill path on a NULL dereference.
   ...
   the first page raises #PF.  The CPU raises #PF, the kernel sees a
   user-mode fault, tears down the PD, and re-enters shell_reload.
   Pairs with the `nullderef` entry in tests/test_programs.py.
