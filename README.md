...
- [2026-06-01 — cc.py `Place` refactor](./2026-06-01-cc-place-refactor-design.md)
  ...
  Plan 5 / Stage 3b design: [2026-06-03-cc-place-refactor-plan5-stage3b-design.md](./2026-06-03-cc-place-refactor-plan5-stage3b-design.md) — operand lowering: introduce optimizer-visible `Address`/`Load`/`Store`/`AddressOf` ops (the IR twin of 3a's `MemoryOperand`, index as a `Value`), fold the **full** access family off `Block`/`Access`, fold + **delete** `ir.Index`/`ir.IndexAssign`, and **rewrite the rep-string matcher** onto the uniform shapes. Two gated PRs: **3b.1** byte-neutral substrate (emission folds `Load(Address)` back to identical x86); **3b.2** the `Index` fold + matcher rewrite (highest risk). CSE/LICM/strength-reduction over `Address` is Stage 3c; retiring `Block` is Stage 4.
