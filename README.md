...
- [2026-06-01 — cc.py `Place` refactor](./2026-06-01-cc-place-refactor-design.md)
  ...
  Plan 5 / Stage 3a design: [2026-06-02-cc-place-refactor-plan5-stage3a-design.md](./2026-06-02-cc-place-refactor-plan5-stage3a-design.md) — recursion-first sub-stage: replace the five bespoke place-codegen emitters with one recursive `resolve_address(place)` over a generalized `MemOperand` (base = label|frame|register), delivering arbitrary-depth `a[i][j][k]`/`a->b[1][2]`/`(*p)[i][j]` and retiring the 2-level `_emit_double_index_place_*` hack. Deref breaks the chain; bitfields/width at the terminal; register choice free; gate = per-function bytes ≤ baseline (or notable perf win). `ir.Index` untouched (3b). Consistent with Clang `EmitLValue` / GCC `get_inner_reference`.
  Plan 5 / Stage 3a plan: [2026-06-02-cc-place-refactor-plan5-stage3a.md](./2026-06-02-cc-place-refactor-plan5-stage3a.md) — incremental gate-driven migration: build `MemOperand` + `resolve_address` alongside the bespoke emitters, migrate one Place shape per task (deref → double-index → member-scalar/index → struct-array), gating per-function byte size + regenerated golden + arbitrary-depth runtime probes after each, deleting each bespoke emitter once its shape is covered. 9 tasks.
