...
class X86CodeGenerator(BuiltinsMixin, CodeGeneratorBase):
    """Generates NASM x86 assembly from the parsed AST.

    Composed from concern-specific mixins (``BuiltinsMixin``, …)
    alongside the arch-agnostic ``CodeGeneratorBase``.  Put the
    mixins before ``CodeGeneratorBase`` so MRO resolves their
    overrides first, though none of them override base methods
    today.
    """
