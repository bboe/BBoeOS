...
class Builder:
    ...
    def _build_do_while(
        ...
        *,
        ...
        self._build_stmts(break_tgt=end_lbl, cont_tgt=cond_lbl, out=out, stmts=body, strings=strings)
        out.extend([
        ...
        *,
        ...
        match expr:
            ...
            case ast_nodes.PlaceLoad(place=place) if _is_nested_subscript_load(expr):
                ...
                indices = self._lower_subscript_chain_indices(out=out, place=place, strings=strings)
                address_temp = self._tmp()
                result_temp = self._tmp()
                out.extend([
    ...
    def _build_for(
        ...
        *,
        ...
        self._build_stmts(break_tgt=None, cont_tgt=None, out=out, stmts=init, strings=strings)
        ...
        self._build_stmts(break_tgt=end_lbl, cont_tgt=step_lbl, out=out, stmts=body, strings=strings)
        out.extend([
    ...
    def _build_if(
        ...
        *,
        ...
        if else_body is not None:
            ...
            self._build_stmts(break_tgt=break_tgt, cont_tgt=cont_tgt, out=out, stmts=body, strings=strings)
            out.extend([Jump(target=end_lbl), Label(name=else_lbl)])
            self._build_stmts(break_tgt=break_tgt, cont_tgt=cont_tgt, out=out, stmts=else_body, strings=strings)
            out.append(Label(name=end_lbl))
        else:
            end_lbl = self._label("endif")
            self._build_cond_false(cond=cond, out=out, strings=strings, target=end_lbl)
    ...
    def _build_switch(
        ...
        *,
        ...
        for case in cases:
            ...
            self._build_stmts(break_tgt=end_lbl, cont_tgt=cont_tgt, out=case_body, stmts=case.body, strings=strings)
    ...
    def _build_while(
        ...
        *,
        ...
        self._build_stmts(break_tgt=end_lbl, cont_tgt=loop_lbl, out=out, stmts=body, strings=strings)
