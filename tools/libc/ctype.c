...
int isalnum(int c) {
    return (unsigned)c < 256 && (_ctype[c] & (_U | _L | _D));
}
int isalpha(int c) {
    return (unsigned)c < 256 && (_ctype[c] & (_U | _L));
}
int iscntrl(int c) {
    return (unsigned)c < 256 && (_ctype[c] & _C);
}
int isdigit(int c) {
    return (unsigned)c < 256 && (_ctype[c] & _D);
}
int islower(int c) {
    return (unsigned)c < 256 && (_ctype[c] & _L);
}
int isprint(int c) {
    return (unsigned)c < 256 && !(_ctype[c] & _C);
}
int ispunct(int c) {
    return (unsigned)c < 256 && (_ctype[c] & _P);
}
int isspace(int c) {
    return (unsigned)c < 256 && (_ctype[c] & _S);
}
int isupper(int c) {
    return (unsigned)c < 256 && (_ctype[c] & _U);
}
int isxdigit(int c) {
    return (unsigned)c < 256 && (_ctype[c] & _X);
}
int tolower(int c) {
