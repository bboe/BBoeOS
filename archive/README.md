...
**asm (+10):** Phase 1 port wraps the remaining NASM source in a
...
`flush_output`, two `die_*` helpers, and the full `main` into pure
C; eight dead `.error_*` labels and ten MESSAGE_* strings retired
along the way.  Follow-up PRs will extract the symbol table, emit
functions, and each instruction-handler family into pure C.
