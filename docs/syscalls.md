...
| 12h   | io_fstat     | Get file status, BX = fd; AL = mode, CX:DX = size     |
| 13h   | io_ioctl     | Device control, BX = fd, AL = cmd, args in other regs per (fd_type, cmd); CF on err |
| 14h   | io_open      | Open file, SI = filename, AL = flags, DL = mode; AX = fd, CF on err |
| 15h   | io_read      | Read from fd, BX = fd, DI = buf, CX = count; AX = bytes, CF on err |
| 16h   | io_seek      | Seek file fd, BX = fd, ECX = offset, AL = whence (SEEK_SET=0, SEEK_CUR=1, SEEK_END=2); EAX = new position (clamped to [0, size]), CF on err |
