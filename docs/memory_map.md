...
| `KERNEL_LOAD_PHYS + .text_size..KERNEL_RESERVED_BASE` | `0xFF82xxxx..` | ~55 KB (today: `0xDF58`) | kernel BSS — hand-written kernel.asm/entry.asm reservations (`program_state_a/b/c`, `parent_iret_frame`, `tss_data`, FD slot, pipeline flags, OOM pending pointers, `sector_buffer_storage`, etc.) and cc.py-emitted zero-init globals from `build/kernel-c/**/*.kasm` (`_g_pipe_pool`, `_g_vga_scrollback_*`, audio rings, etc.); declared `section .bss nobits` in `kernel/arch/x86/kernel.asm` so the bytes do not ride on disk; zero-filled by `high_entry` after the kernel-stack poison-fill, before `lidt`. `make_os.sh` reads `kernel_bss_start` / `kernel_bss_end` from `build/kernel.map` and adds the extent to `KERNEL_RESERVED_BASE` so the kernel stacks (next row) do not overlap. | no |
| `KERNEL_RESERVED_BASE..` | `0xFF838000..` (≈) | 3 × 1 KB | three ring-0 kernel stacks (`kernel_stack` = slot_a, `kernel_stack_b` = pipeline cmd1, `kernel_stack_c` = pipeline cmd2); `KERNEL_STACK_BYTES = 0x400`. `KERNEL_RESERVED_BASE = page_align(0x20000 + kernel_size + bss_size)`; each stack poison-filled with `0xDEADBEEF` at boot for high-water tracking | no |
| ~`0x39000..0x39FFF` | `0xFF839000..` (≈) | 4 KB | boot PD (`BOOT_PD_PHYS = page_align(KERNEL_RESERVED_BASE + 3 * KERNEL_STACK_BYTES)`; the page-align pad covers the sub-page slack left by three 1 KB stacks); freed back to the bitmap pool by `high_entry` after `kernel_idle_pd` takes over the CR3-target role. The slot is then just a regular conventional frame — the bitmap allocator can hand it out for user pages. | no |
| ~`0x3A000..0x3AFFF` | `0xFF83A000..` | 4 KB | first kernel PT (`FIRST_KERNEL_PT_PHYS = BOOT_PD_PHYS + 0x1000`) | no |
| ~`0x3B000..` | `0xFF83B000..` | runtime, ≤ 128 KB | `frame_bitmap` (size set by `frame_init` from the highest type=1 E820 base, clamped to FRAME_PHYSICAL_LIMIT ≈ 4 GB — `-m 1` pays ~20 bytes, `-m 1024` pays 32 KB, `-m 4096` pays 128 KB; `frame_init` fills the storage before any allocator call, so the bytes don't ride on disk inside `kernel.bin`) | no |
| `FRAME_BITMAP_PHYS + frame_bitmap_bytes` | `0xFF800000 +` same | -- | end of the kernel reserve sweep — runtime ceiling, equals `0x3B000 + 20 B` on `-m 1`, `0x3B000 + 32 KB` on `-m 1024`, and `0x3B000 + 128 KB` on `-m 4096` (these offsets shift with `kernel.bin` + BSS size; see the build-time derivation); everything past this in conventional RAM is owned by the bitmap allocator (subject to E820's reserved regions, including the VGA aperture at `0xA0000..0xFFFFF`) | -- |
| dynamic | dynamic | 4 KB (ext2 only) | ext2 directory-window frame — allocated by `ext2_init` only when the superblock magic matches; holds `ext2_search_blk`'s sliding 2-sector directory window (1 KB used in the 4 KB frame; the upper 3 KB sits unused — no sub-page allocator). bbfs systems never spend this frame; `ext2_sd_buffer` stays NULL. `sector_buffer` (the 512 B scratch every disk read writes through) now lives in the kernel BSS as `sector_buffer_storage` (see the BSS row above), unconditionally available on every boot. `ext2_init` treats frame_alloc failure here as a hard panic. | no |
