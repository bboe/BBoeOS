...
handle_inc:
        call skip_ws
        call parse_register    ; AH = size, AL = reg
        cmp ah, 8
        je .inc_r8
        ;; inc r16: 40+reg
        add al, 40h
...
handle_mov:
        ...
        .mov_rm_direct:
        ;; mov r, [disp16]: 8B (16-bit) or 8A (8-bit)
        ;; modrm: mod=00, reg=dst, rm=110
        cmp byte [op1_size], 8
        je .mov_rm_d8
        mov al, 8Bh
        jmp .mov_rm_d_emit
...
handle_sub:
        ...
        mov cx, ax
        pop bx                 ; BL = reg
        ;; Emit: 81 modrm(mod=11, /5, rm=reg) imm16
        mov al, 81h
        call emit_byte_al
        ;; modrm = (3 << 6) | (5 << 3) | rm = C0 | 28 | rm = E8 | rm
        mov al, bl
        or al, 0E8h
        call emit_byte_al
        mov ax, cx
