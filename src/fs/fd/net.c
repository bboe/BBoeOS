...
// reached (the asm `mov [fd_write_buffer], esi` stash).  asm_name skips
// cc.py's storage emission; the actual byte-slot lives in fd.c.
uint8_t *fd_write_buffer __attribute__((asm_name("fd_write_buffer")));
