...
;;; advancing after the pmode switch lands — the PIT keeps its default
;;; ~18.2 Hz rate, giving ~55 ms resolution.  The existing callers already
;;; divide ticks by 18 to get seconds, so they are unchanged.
;;; ------------------------------------------------------------------------
        ...
        TICKS_PER_SECOND        equ 18          ; approx (BIOS default rate)
...
rtc_sleep_ms:
        ...
        ;; Preserves all registers.  55 ms granularity (one PIT tick).
        ...
        push edx
        movzx eax, cx
        add eax, 54             ; round up to whole ticks
        xor edx, edx
        mov ebx, 55
...
rtc_tick_init:
        ;; Install our own IRQ 0 handler into the real-mode IVT and zero
        ;; the tick counter.  Call once, early in stage 2 boot.
        cli
        push ax
        push es
        xor ax, ax
