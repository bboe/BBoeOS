...
// fs/fd.c (currently still asm) returns AX = fd number, ESI = entry
// pointer, CF set on failure.
...
// drivers/ne2k.c file-scope globals exposed via `equ _g_*` aliases
// there.  asm_name skips cc.py's storage emission and points the C
// reference at the ne2k.c-owned bytes directly.  cc.py doesn't accept
// asm_name on array declarations, so mac_address surfaces as a single-
// byte alias and `sys_net_mac` reads it as a 6-byte block via &.
