...
### Networking
- Convert the network-stack aggregator `src/net/net.asm` (4 lines of `%include` directives) into `src/net/net.c` containing the same directives wrapped in `asm("...")` escapes.  cc.py compiles it to `build/kernel-c/net/net.kasm`; `src/arch/x86/kernel.asm` now `%include`s `net/net.kasm` instead of `net/net.asm`.  This finishes the network port: every file under `src/net/` is now C — `arp.c`, `icmp.c`, `ip.c`, `udp.c`, plus the `net.c` orchestrator.  The same trick (a `.c` file whose body is `asm("%include ...")` lines) generalises to any "subsystem aggregator" that just lists its modules — useful as more `src/{drivers,fs,...}/*.asm` modules move to C and want to keep their per-subsystem grouping without forcing `kernel.asm` to know each module by name.
- Port `arp_handle_packet` / `arp_resolve` / `arp_send_request` / `arp_table_add` / `arp_table_lookup` from `net/arp.asm` to C in a new `src/net/arp.c`, retiring the asm file.  Internal storage uses a `struct arp_entry { uint8_t ip[4]; uint8_t mac[6]; uint16_t timestamp; }` array (8 entries) with round-robin eviction once full and the asm version's "first empty slot ⇒ stop search" optimisation preserved.  Frame construction uses `memcpy` / `memset` (cc.py builtins) instead of the original's `stosb` / `stosw` / `movsd` chains.  Two cc.py kinks worked around in the C version: (1) `arp_resolve`'s timeout counter uses `while (timeout != 0)` instead of `while (timeout > 0)` because cc.py emits a signed compare for `>` and `0xFFFF` reads as -1, exiting the loop immediately; (2) `arp_table_lookup` calls `uptime_seconds()` at the candidate-match site instead of hoisting it to a `now` local because cc.py auto-pins that local to BX, which the struct-deref base-register pattern then clobbers without invalidating the pin.  The latter is a pre-existing cc.py bug worth fixing later — the workaround keeps arp.c clean.
...
### Syscalls
- Consolidate the INT 30h dispatcher: inline every per-syscall handler that lived in `src/syscall/{fs,io,net,rtc,sys}.asm` directly into `arch/x86/syscall.asm`, retiring those five files.  The trivial pass-through handlers (every `fs_*` / `io_*` / `rtc_*` plus `sys_exec` / `sys_exit` / `sys_reboot` / `sys_shutdown`) are 2-4 lines of asm each — `call <existing_function>; jmp .iret_cf` (or `.iret_done` for the rtc handlers that explicitly populate saved AX/DX slots) — and gain nothing from a separate `%include`.  The four non-trivial network handlers (`sys_net_mac` / `sys_net_open` / `sys_net_recvfrom` / `sys_net_sendto`) move to a new `src/syscall/syscalls.c`: `sys_net_mac` becomes `memcpy(out, mac_address, 6)` after a `net_present` check; `sys_net_open` is `fd_alloc` followed by an `(SOCK_DGRAM, IPPROTO_*)` → `FD_TYPE_*` table; `sys_net_recvfrom` walks the fd table, dispatches into `udp_receive` / `icmp_receive`, port-filters the UDP path against `[NET_RECEIVE_BUFFER+36]`, and `memcpy`s the clamped payload into the user buffer; `sys_net_sendto` walks the fd table, takes `dst_port` via `in_register("ax")` (the dispatcher pre-loads it from the saved BP slot at `[bp+4]`), and dispatches into `udp_send` / `ip_send`.  Behaviour-identical: `netrecv` / `netsend` / `ping` and all 27 program runtime tests stay green; binary sizes unchanged on the user side; the `bin/shell` shell-protect contract for `chmod` / `rename` / `unlink` is preserved (the `.check_shell` helper still lives in the dispatcher).

### Drivers
- Port `vga_clear_screen` / `vga_fill_block` / `vga_get_cursor` / `vga_scroll_up` (now `vga_text_scroll_up`) / `vga_set_bg` / `vga_set_cursor` / `vga_set_mode` / `vga_set_palette_color` / `vga_teletype` / `vga_write_attribute` / `fd_ioctl_vga` from `drivers/vga.asm` to C in a new `src/drivers/vga.c`, retiring the asm file.  CRTC / AC / Sequencer / Graphics-Controller register banging is straight C against `kernel_inb` / `kernel_outb`; the framebuffer-touching primitives (`vga_text_clear_screen` / `vga_text_scroll_up` / `vga_text_putw` / `vga_graphics_clear_screen` / `vga_graphics_fill_8x8` / `vga_font_load`) live in a file-scope `asm("...")` block at the bottom because cc.py's pure C path can't reach 0xB8000 / 0xA0000 in 16-bit real mode (DS=0 only covers 0..0xFFFF, so each one needs an `ES = 0xB800` / `0xA000` segment override that pure C can't emit).  `vga_set_mode`'s mode-id table drops the per-entry mode-id byte (mode dispatch is now an `if/else if` ladder in C) and consolidates the two 61-byte entries into one flat 120-byte `uint8_t vga_mode_table[120]`; `vga_default_palette[48]` and `vga_current_mode` are plain C globals with cc.py's `_g_` prefix.  `ansi_fg` (read by `vga_teletype` / `vga_write_attribute`) is now a plain C global in `ansi.c` (the previous bare-name asm() block is gone) — `vga.c` reaches it via an `asm_name("_g_ansi_fg")` alias, matching the `mac_address` / `net_present` pattern.  Bug fix: `fd_ioctl_vga` now declares `CX` and `DX` as explicit `in_register` parameters so the user's `row_col` / `dx_arg` survive `fd_ioctl`'s call into `fd_lookup` (which clobbers DX) — the pre-port code path was latently broken for `VGA_IOCTL_FILL_BLOCK` / `VGA_IOCTL_MODE` / `VGA_IOCTL_SET_PALETTE` since the C port of `fd_ioctl` landed; nothing in the automated test suite exercised those paths so it never fired, but `Ctrl+L` and `draw` would have seen garbage args.  `fd_ioctl` is correspondingly extended with the same two extra in_register params, spilling them to its frame so they're alive after `fd_lookup`.  cc.py extension: the three `VGA_IOCTL_*` names are added to `NAMED_CONSTANTS` so they resolve in expression context (alongside the existing `VIDEO_MODE_*`, `FD_TYPE_*`, `IPPROTO_*`, etc.).  All 35 self-host + 17 archive + 5 fs + 27 program + 38 ext2 + 135 kernel-C tests stay green; manual smoke through `python3 tests/run_qemu.py "ls" "echo abc" "uptime"` shows the welcome banner, prompt, and command output rendering normally.

- Port `ne2k_init` / `ne2k_probe` / `ne2k_receive` / `ne2k_send` / `network_initialize` from `drivers/ne2k.asm` to C in a new `src/drivers/ne2k.c`, retiring the asm file.  The polled NE2000 ISA driver (RX ring at pages 0x46..0x7F, TX buffer at pages 0x40..0x45, all packet movement via remote DMA through the data port at 0x310) becomes ~360 lines of straight-line C against `kernel_inb` / `kernel_outb` / `kernel_inw` / `kernel_insw` / `kernel_outsw`.  All `rep insw` / `rep outsw` blocks for packet body transfer collapse to single-builtin calls.  Per-register `NE_*` constants use full literal port addresses (`NE_CR=0x300`, `NE_DATA=0x310`, etc.) rather than `NE2K_BASE+offset` so cc.py's `#define`-emitted `%define` doesn't collide with the `%assign NE2K_BASE 300h` in `include/constants.asm` — same trap that has now hit ps2.c (PIC1_DATA), ansi.c (VGA_COLS), and fdc.c (DMA_*).  Page-1 register names (`NE_PAR0`, `NE_CURR`, `NE_MAR0`) get separate aliases for the same port numbers as page-0 PSTART / ISR / RSAR0 — clearer than runtime page-index arithmetic.  `mac_address` and `net_present` are plain C globals (`uint8_t mac_address[6]; uint8_t net_present;`); cc.py emits storage with its standard `_g_`-prefix and ne2k_init / ne2k_probe index `mac_address[i]` directly.  The existing aliases in `arp.c` / `ip.c` / `syscalls.c` (`uint8_t mac_address_ref __attribute__((asm_name("mac_address"))));` and friends) retarget to `asm_name("_g_mac_address")` / `asm_name("_g_net_present")` — cc.py rejects `asm_name` on array declarations and has no real `extern` keyword, so the consumer side aliases a single byte at the array's base address (pointer arithmetic gets the rest if needed; the syscall handler's `memcpy(out, &mac_address_byte0, 6)` already worked through that base).  Verified end-to-end via the existing test_programs.py ping + DNS suite plus a manual ICMP exchange with the QEMU SLIRP gateway (`netinit; ping 10.0.2.2` → 4×"Reply from 10.0.2.2: time=0 ms").  All 35 self-host + 17 archive + 5 fs + 27 program + 38 ext2 + 135 kernel-C tests stay green.

- Port `fdc_init` / `fdc_read_sector` / `fdc_write_sector` (plus `fdc_send` / `fdc_recv` / `fdc_dma_setup` / `fdc_seek` / `fdc_issue_read_write` / `fdc_lba_to_chs` / `fdc_drain_result` / `fdc_sense_interrupt`) from `drivers/fdc.asm` to C in a new `src/drivers/fdc.c`, retiring the asm file.  The native floppy driver (8237 DMA channel 2 + IRQ 6 for command completion, 1.44 MB CHS geometry) becomes ~250 lines of straight-line C against the cc.py `kernel_inb` / `kernel_outb` builtins; the only inline asm left is a file-scope `asm("...")` block at the bottom that holds the three pieces cc.py can't express: `fdc_irq6_handler` (must `iret`), `fdc_install_irq` (writes the IVT entry at `[es:0x26*4]` and toggles the PIC mask via `cli` / `sti`-bracketed IN/OUT), and `fdc_wait_irq` (the `pushf` / `sti` ... `popf` busy-wait that has to preserve the caller's IF state — important because the syscall dispatcher enters with IF=0).  `fdc_irq_flag`'s storage lives in that same asm block with a `uint8_t fdc_irq_flag __attribute__((asm_name("fdc_irq_flag")))` C alias so reads / writes from C and from the ISR target the same byte.  CHS coordinates (cylinder / sector / head) round-trip through three `uint8_t` module globals (`fdc_chs_cyl` / `fdc_chs_sector` / `fdc_chs_head`) instead of out_register pointers because cc.py's `*ptr = val` write only supports out_register parameters and "return three numbers" is cleaner via module state than three register-pinned outputs.  Constants are `FDC_*` prefixed (`FDC_DMA_MASK` instead of bare `DMA_MASK`, `FDC_MSR_RQM` instead of `MSR_RQM`) for the same reason ps2.c needed `PS2_*` and ansi.c needed `ANSI_*`: cc.py emits `#define` as a NASM `%define` macro that mass-substitutes throughout subsequent translation units.  `SECTOR_BUFFER`'s low / high bytes are hardcoded as `0x00` / `0xE0` rather than `SECTOR_BUFFER & 0xFF` / `(SECTOR_BUFFER >> 8) & 0xFF` because cc.py doesn't constant-fold extern-symbol arithmetic and would emit four runtime instructions per DMA address poke.  Verified end-to-end by booting `qemu-system-i386 -drive file=drive.img,format=raw,if=floppy`: shell loads cleanly through `fdc_read_sector`, `mkdir testdir` then `ls` confirms `fdc_write_sector` (subdirectory entry persists across the read-back).  Default ATA tests stay green too; all 35 self-host + 17 archive + 5 fs + 27 program + 38 ext2 + 135 kernel-C tests still pass.

- Port `put_character` / `put_string` / `serial_character` from `drivers/ansi.asm` to C in a new `src/drivers/ansi.c`, retiring the asm file.  The full ANSI escape parser (NORMAL → ESC → CSI state machine, 256-color SGR, cursor up / forward / back / position, `ESC[<N>@` write-attribute) becomes ~150 lines of straight-line C against an `int ansi_params[3]` array indexed by a word-sized `ansi_param_index` (the asm version's byte-offset indexing collapses to natural word indexing because cc.py scales array indices by element size).  vga.asm's CRTC / attribute-controller helpers stay asm and get C externs: `vga_get_cursor` returns DH=row / DL=col packed into an `int` via `out_register("dx")`, `vga_set_cursor` takes the same packing as `in_register("dx")`, and `vga_write_attribute` takes char in AX + attribute in BX.  `serial_character` is a `kernel_inb` / `kernel_outb` poll loop with `preserve_register("ax")` + `preserve_register("dx")` to keep the asm-version's caller contract.  `put_character` carries `preserve_register` for AX / BX / CX / DX (matching the asm push/pop fan); `put_string` carries `preserve_register("ax")` so asm callers in stage2.asm see their AL untouched.  `ANSI_COLS` is renamed from `VGA_COLS` to avoid the same `%define` vs `equ` collision that bit ps2.c — vga.asm's `VGA_COLS equ 80` would otherwise expand to `80 equ 80` after macro substitution.  `ansi_fg`'s storage lives in a file-scope `asm("ansi_fg db 7")` block with `asm_name("ansi_fg")` aliasing the C variable, so vga.asm's existing `[ansi_fg]` reads (in `vga_teletype` / `vga_write_attribute`) keep working without renaming to `_g_ansi_fg`.  Behaviour-identical at runtime: every console write (welcome banner, shell prompt, command output, ANSI line-editor sequences for cursor movement) flows through this code; all 35 self-hosted assembler tests, 17 archive size tests, 5 fs tests, 27 program tests, and 135 kernel-C unit tests stay green.
- Port `ps2_init` / `ps2_check` / `ps2_read` / `ps2_service` / `ps2_read_scancode` from `drivers/ps2.asm` to C in a new `src/drivers/ps2.c`, retiring the asm file.  The 8042 polling driver (Set-1 scan codes → ASCII via two 59-byte keymap arrays, with shift / ctrl / extended-prefix state machines) becomes ~150 lines of straight-line C.  Both keymap arrays are file-scope `uint8_t[]` initializers carrying the same byte sequences NASM emitted via stacked `db` directives.  `ps2_check`'s contract changes from ZF-returning to `carry_return` (CF clear = key ready, CF set = no key) — fd.c's old 5-line `ps2_has_key` ZF-to-CF wrapper is gone, and the call site in `fd_read_console` becomes a direct `if (!ps2_check())`.  Constants are `PS2_*` prefixed (`PS2_PIC1_DATA` instead of `PIC1_DATA`) because cc.py's `#define` expands as a NASM `%define` macro and would otherwise collide with the `equ` definitions in `drivers/rtc.asm` (later in the same translation unit) — `PIC1_DATA equ 21h` after macro substitution becomes `0x21 equ 21h` and refuses to assemble.  Behaviour-identical at runtime: every keystroke through `fd_read_console` (i.e. all 27 program runtime tests) flows through this code path.
- Port `ata_issue` / `ata_wait_drq` / `ata_read_sector` / `ata_write_sector` from `drivers/ata.asm` to C in a new `src/drivers/ata.c`, retiring the asm file.  The native ATA PIO driver (primary controller, master drive, LBA28) talks to ports `0x1F0`-`0x1F7` via the cc.py `inb` / `outb` builtins; the only inline asm left is the two `rep insw` / `rep outsw` blocks that move a sector through DX as fast as the bus allows (cc.py has no string-I/O builtins, and a per-word loop would lose ~50% of the throughput).  Each public function carries a `preserve_register` attribute per register the asm version push / popped (BX / CX / DX / DI for read; BX / CX / DX / SI for write) so callers in fs/bbfs.asm and fs/ext2.asm see the same callee-saved contract.  AX is the in_register input — its post-call value isn't part of the contract since every caller reloads it before reuse.  Internal helpers (`ata_issue` / `ata_wait_drq`) stay private to the file but follow the same convention.  Exercised by `test_bboefs.py copy_large` (copies a >64KB file so multiple LBA28 sectors flow through both directions) plus all 17 archive size tests, 5 fs regressions, and 27 program runtime tests.
...
### Library
- Port `shared_die` / `shared_exit` / `shared_get_character` / `shared_parse_argv` from `lib/proc.asm` to C in a new `src/lib/proc.c`, retiring the asm file.  The first three are thin INT 30h syscall thunks autodetected by cc.py as `naked_asm` (no parameters, single `asm()` body): the prologue / epilogue is elided and the body is emitted verbatim, with `peephole_dead_code` stripping the auto-appended `ret` after `shared_die`'s `jmp shared_exit`.  `shared_parse_argv` is now a real C function — `uint8_t **argv` arrives in DI via `in_register`, `int *argc` returns count via `out_register("cx")` (matching the existing FUNCTION_PARSE_ARGV asm-side caller's CX = argc convention), and the body is a straight argv tokenizer (skip-spaces, record-start, find-end, null-terminate, advance).  `uint8_t *exec_arg_pointer __attribute__((asm_name("EXEC_ARG")))` aliases the [EXEC_ARG] memory slot so the C code reads the original arg-string pointer the same way `mov si, [EXEC_ARG]` did.  Trade-off: the C version compiles to ~80 instructions vs the asm version's ~27 (cc.py's IR-based codegen materialises each intermediate to a stack slot), and `shared_die` adds a 3-byte explicit `jmp shared_exit` instead of asm-style label fall-through.  Behaviour-identical: all 35 program tests pass.

### Filesystem
- Port `read_sector` / `write_sector` from `fs/block.asm` to pure C in a new `src/fs/block.c`, retiring the asm file.  Each wrapper is declared `__attribute__((carry_return)) __attribute__((naked))` with the sector LBA pinned to AX via `in_register("ax")`; cc.py emits no prologue / epilogue, threads the if/else through tail-call detection so each branch ends in a `jmp`, and uses the unsigned compare path because `boot_disk` is `uint8_t`.  The peephole optimizer collapses the resulting `jae .else ; jmp fdc ; .else: jmp ata` into `jb fdc ; jmp ata`, producing the same 3-instruction `cmp / jb / jmp` dispatcher the asm version emitted by hand — byte-identical output, pure C source.
- Port `fd_read_net` / `fd_write_net` from `fs/fd/net.asm` to C inside `src/fs/fd.c`, retiring the asm file.  Both functions now use the established carry_return + out_register("ax") pattern (matching `fd_alloc` / `fd_open`); `fd_read_net` calls `ne2k_receive` (declared with `out_register("di"))` for the frame pointer plus a new `out_register("cx")` for packet length) and `memcpy`s the clamped payload into the user buffer; `fd_write_net` calls `ne2k_send(fd_write_buffer, count)` and surfaces AX=count / AX=-1 + CF on error.  All 27 program tests (incl. `netrecv`, `netsend`, `ping`) and 94 kernel-C unit tests stay green.
- Port `fd_read_file` / `fd_write_file` from `fs/fd/fs.asm` to C inside `src/fs/fd.c`, retiring the asm file.  Both functions loop over sectors via `vfs_read_sec` / `vfs_prepare_write_sec` / `vfs_commit_write_sec` (now declared as carry_return externs with `out_register("bx")` for the byte offset), copying `min(512 - byte_offset, left)` bytes per iteration with `memcpy`.  EOF is checked via two 16-bit `pos_hi`/`size_hi` comparisons (avoiding 32-bit subtract); when `position_hi == size_hi` the count is clamped to `size_lo - position_lo`.  Position is advanced via a tiny `fd_advance_position` asm helper (3 instructions: `add [si+POS], cx; adc word [si+POS+2], 0; ret`) — pure C cannot express add-with-carry through cc.py's signed-only `<` codegen, and a helper is cleaner than spilling a per-call inline-asm escape.  The redundant `fd_read_dir` wrapper (was just `jmp vfs_read_dir`) is removed; `fd_ops[FD_TYPE_DIRECTORY]` now points at `vfs_read_dir` directly.  Exercised by 38 ext2 tests (incl. `doubly_indirect_cat`/`_shrink` on 280 KB files that hit the `position_hi != 0` branch) and 5 bbfs tests (incl. `copy_large` on >64 KB files).
- Port `fd_read_console` / `fd_write_console` from `fs/fd/console.asm` to C inside `src/fs/fd.c`, retiring the asm file (and the `src/fs/fd/` directory entirely).  Polling loop uses the new `inb` / `outb` cc.py builtins so the COM1 LSR (3FDh) and data (3F8h) reads compile to the same `in al, dx` / `xor ah, ah` the asm version emitted by hand.  PS/2 plumbing keeps drivers/ps2.asm untouched: a 7-line `ps2_has_key` carry_return wrapper in fd.c's asm block bridges `ps2_check`'s ZF-based contract to cc.py's CF-based one, and `ps2_read` is declared as `int` so callers can split the AL/AH-packed return into `ascii = packed & 0xFF` / `scan_code = (packed >> 8) & 0xFF`.  Extended-key handling (arrows → ESC '[' A/B/C/D) and the dead-but-preserved 2-slot serial pushback FIFO (drain only, no fill site exists) match the asm version exactly.  The `sti` at the top of the polling loop ports as `asm("sti")` so PIT IRQ 0 keeps advancing `system_ticks` while the shell idles.  Every program test exercises this path (input reads + put_character writes for echo / prompt / output).
