...
- Port the VGA driver (text-mode framebuffer, mode 13h, CRTC cursor pair, mode programming, `/dev/vga` ioctl backend) from `drivers/vga.asm` to C in a new `src/drivers/vga.c`.  C-shaped pieces: `vga_get_cursor`, `vga_set_cursor`, `vga_set_bg`, `vga_set_palette_color`, and `vga_fill_block`'s 8x8 mode-13h tile loop using the new `far_write8` builtin for `0xA0000`-region writes.  The remaining pieces stay in file-scope `asm()` blocks because cc.py can't express their shapes: `vga_clear_screen` / `vga_scroll_up` (`rep stosw` over 2000 cells), `vga_teletype` / `vga_write_attribute` (per-cell framebuffer writes that would balloon to a call frame per character), `vga_set_mode` (table-walking `lodsb`-driven traversal of the 122-byte `vga_mode_table`), and `fd_ioctl_vga` (the syscall jump-table dispatch entry that needs CF-based return contracts cc.py doesn't model).

### Filesystem
- Port `fs/fd/console.asm` (PS/2 + COM1 polled console fd backend) to C in a new `src/fs/fd/console.c`, retiring the asm subfile.  Two functions: `fd_read_console` polls `ps2_getc` and COM1's data-ready bit (port `0x3FD`) for one byte, CR→LF translates, and stores at the user `EDI`; `fd_write_console` loops `put_character` over the buffer at `[fd_write_buffer]`.  The C version `asm("sti")`s once before the polling loop (the syscall handler enters with IF=0 so IRQ 1 needs to be enabled for the keyboard ring to populate).  cc.py rejects `*destination = byte` on non-`out_register` pointer parameters, so the C port writes `destination[0] = byte` instead — same single-byte `mov [edi], al` either way.

- Port `fs/fd/net.asm` (raw NE2000 fd backend) to C in a new `src/fs/fd/net.c`, retiring the asm subfile.  Two functions: `fd_read_net` polls `ne2k_receive` and memcpys the frame to the user buffer; `fd_write_net` sends the buffer at `[fd_write_buffer]` via `ne2k_send`.  Three changes that unblock further `fs/fd/*.asm` ports: tightening `ne2k_receive`'s C declaration with `out_register("edi")` / `out_register("ecx")` + `carry_return` so the multi-register output captures cleanly into C variables; lifting `fd_write_buffer` out of `fs/fd.c`'s file-scope `asm()` block to a C-level `uint8_t *` global with an `equ` shim so the surviving `fs/fd/{console,fs}.asm` references resolve unchanged; and extending `tools/measure_kernel_ports.sh` with a per-row "host file" field so swap-build measurements work for ports whose `%include` lives in a `.c` file's `asm()` block (not just `bboeos.asm`).

### Syscall
- Consolidate the INT 30h dispatcher and port the four non-trivial network handlers to C.  The five `src/syscall/{fs,io,net,rtc,sys}.asm` subfiles are retired — every trivial pass-through (`fs_*`, `io_*`, `rtc_*`, `sys_exec` / `sys_exit` / `sys_reboot` / `sys_shutdown`) is inlined directly into `src/arch/x86/syscall.asm` since each is 2-4 lines (`call <existing_function>; jmp .iret_cf`) and gained nothing from a separate `%include` or C wrapper.  The four non-trivial network handlers (`sys_net_mac`, `sys_net_open`, `sys_net_recvfrom`, `sys_net_sendto`) move to a new `src/syscall/syscalls.c`: real branching on `fd_lookup`'s type, payload `memcpy` from `udp_receive` / `icmp_receive`, per-protocol dispatch into `udp_send` / `ip_send`.  cc.py's `carry_return` plus `out_register("ax")` keeps the asm-side `AX = result` / `CF = error` return convention.  `sys_net_sendto` receives the user's `dst_port` via AX — the dispatcher pre-loads it from `[esp+8]` (the saved-EBP slot in the pushad frame) before the call.  `check_shell` stays in the dispatcher as a local helper.
