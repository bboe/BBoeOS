...
- Self-hosted assembler (`src/c/asm.c`): `handle_sub` picks the 5-byte `83 /5 ib` (sign-extended imm8) short form for `sub word [disp16], imm8` instead of always emitting the 6-byte `81 /5 iw` form, matching NASM byte-for-byte on the shape `cc.py`'s `peephole_memory_arithmetic` produces for `x -= N` when `x` is memory-allocated.  `bits.c` picks up a `y -= 5` smoke test that exercises the fixed encoder; `test_asm.py` (reassemble-and-diff) catches any regression
