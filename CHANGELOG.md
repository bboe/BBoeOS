...
### Drivers
- Port `ne2k_init` / `ne2k_probe` / `ne2k_receive` / `ne2k_send` / `network_initialize` from `drivers/ne2k.asm` to C in a new `src/drivers/ne2k.c`, retiring the asm file.  The polled NE2000 ISA driver (RX ring at pages 0x46..0x7F, TX buffer at pages 0x40..0x45, all packet movement via remote DMA through the data port at 0x310) becomes ~360 lines of straight-line C against `kernel_inb` / `kernel_outb` / `kernel_inw` / `kernel_insw` / `kernel_outsw`.  All `rep insw` / `rep outsw` blocks for packet body transfer collapse to single-builtin calls.  Per-register `NE_*` constants use full literal port addresses (`NE_CR=0x300`, `NE_DATA=0x310`, etc.) rather than `NE2K_BASE+offset` so cc.py's `#define`-emitted `%define` doesn't collide with the `%assign NE2K_BASE 300h` in `include/constants.asm` — same trap that has now hit ps2.c (PIC1_DATA), ansi.c (VGA_COLS), and fdc.c (DMA_*).  Page-1 register names (`NE_PAR0`, `NE_CURR`, `NE_MAR0`) get separate aliases for the same port numbers as page-0 PSTART / ISR / RSAR0 — clearer than runtime page-index arithmetic.  `mac_address` (6 bytes) and `net_present` (1 byte) storage lives in a file-scope `asm("...")` block at the bottom of `ne2k.c`; C aliases use `asm_name` to keep the bare NASM symbol names so the existing aliases in `arp.c` / `ip.c` / `syscalls.c` (which already used `asm_name("mac_address")` and `asm_name("net_present")`) continue to resolve.  cc.py only allows `*ptr = val` writes through `out_register` parameters, so MAC byte access in the loop bodies of `ne2k_init` (writing PAR0..PAR5) and `ne2k_probe` (reading the 6 PROM bytes) goes through a `uint8_t *mac_pointer = &mac_address_byte0` local that gets `mac_pointer[i]` indexing.  Verified end-to-end via the existing test_programs.py ping + DNS suite plus a manual ICMP exchange with the QEMU SLIRP gateway (`netinit; ping 10.0.2.2` → 4×"Reply from 10.0.2.2: time=0 ms").  All 35 self-host + 17 archive + 5 fs + 27 program + 38 ext2 + 135 kernel-C tests stay green.
