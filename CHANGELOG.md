...
- Self-hosted assembler (`src/c/asm.c`): factor the `<op> word [disp16], imm` parsing and emission into a shared `emit_alu_word_mem_imm(rfield)` helper and extend coverage from `sub` (the only op the old inline in `handle_sub` knew about) to `add`, `and`, `or`, `sub`, `xor`.  Each picks the 5-byte `83 /r ib` sign-extended short form when the immediate fits signed 8-bit, otherwise the 6-byte `81 /r iw` form, matching NASM byte-for-byte on every shape `cc.py`'s `peephole_memory_arithmetic` produces for a memory-allocated variable.  `bits.c` exercises all five via `y -= 5` (memory-allocated local) and a global `counter` stepping through `+=` / `|=` / `&=` / `^=` with printf between each so AX is clobbered and the reload/op/store triple forms (which the peephole then fuses)
