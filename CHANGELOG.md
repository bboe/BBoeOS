...
### Tree layout
- Move the kernel from `src/kernel/` to `src/arch/x86/`, with the bootloader (`bboeos.asm`, `stage1.asm`, `stage2.asm`, `ansi_minimal.asm`) pulled out into `src/arch/x86/boot/`.  Mirrors Linux's `arch/x86/{boot,…}` convention and gives the pmode port a natural home: incremental pmode work lives on a dedicated branch cut from `main` while `main` itself stays bootable.  The old `src/kernel/arch/{idt,pic,pmode}.asm` redundant nesting flattens to `src/arch/x86/{idt,pic,pmode}.asm`.  `make_os.sh` gains two `-i` flags (`-i src/arch/x86/ -i src/arch/x86/boot/`) so stage 2's bare `%include "ansi.asm"` / `%include "drivers/ata.asm"` / … paths still resolve.  `tests/test_pmode.sh` and `tests/test_idt.sh` updated for the new layout; both smoke tests now run green again (they had broken on the earlier `arch/` sub-move)
