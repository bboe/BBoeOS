...
Kernel-side fixed-physical regions, all reached through the kernel direct map at virt `0xC0000000 + phys`.  The "in kernel.bin?" column flags whether the bytes occupy the on-disk image (`yes`) or live as bare frames pre-reserved by `LOW_RESERVE_BYTES` (`no`).  Addresses from `kernel_stack` onward are derived from `KERNEL_RESERVED_BASE = page_align(0x20000 + sizeof(kernel.bin))`; example values shown are for the current build (~38 KB kernel).  Everything below `LOW_RESERVE_BYTES` (~0x51000 today, well under the VGA hole at `0xA0000`) is reserved at boot via `frame_reserve_range`; the build script asserts this stays under `0xA0000` so the OS keeps booting under QEMU `-m 1`.  Update this table when adding a new fixed-phys region so newcomers can find every slot in one place.
...
| `0x00020000..` | `0xC0020000..` | ~38 KB | `kernel.bin` (resident kernel image) | yes |
