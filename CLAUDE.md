...
| `0x00020000..0x00020001` | `0xC0020000..0xC0020001` | 2 B | `jmp short high_entry` trampoline (offset 0 of kernel.bin) | yes |
| `0x00020002` | `0xC0020002` | 1 B | `boot_disk` (BIOS drive number, written by boot.asm post-load) | yes |
| `0x00020003..0x00020004` | `0xC0020003..0xC0020004` | 2 B | `directory_sector` (LBA of first directory sector) | yes |
| `0x00020008..` | `0xC0020008..` | ~29 KB | `kernel.bin` `high_entry` and resident kernel code | yes |
| `KERNEL_RESERVED_BASE` (~`0x28000..0x28FFF`) | `0xC0028000..` | 4 KB | `kernel_stack` (`KERNEL_RESERVED_BASE = page_align(0x20000 + kernel_size)`; poison-filled with `0xDEADBEEF` at boot for high-water tracking) | no |
| ~`0x29000..0x29FFF` | `0xC0029000..` | 4 KB | boot PD (`BOOT_PD_PHYS`); freed back to the bitmap pool by `high_entry` after `kernel_idle_pd` takes over the CR3-target role.  The slot is then just a regular conventional frame — the bitmap allocator can hand it out for user pages. | no |
| ~`0x2A000..0x2AFFF` | `0xC002A000..` | 4 KB | first kernel PT (`FIRST_KERNEL_PT_PHYS`) | no |
| ~`0x2B000..` | `0xC002B000..` | runtime, ≤ 32 KB | `frame_bitmap` (size set by `frame_init` from the highest type=1 E820 base, clamped to 1 GB by LAST_KERNEL_PDE — `-m 1` pays ~20 bytes, `-m 1024` pays 32 KB; `frame_init` fills the storage before any allocator call, so the bytes don't ride on disk inside `kernel.bin`) | no |
| `FRAME_BITMAP_PHYS + frame_bitmap_bytes` | `0xC0000000 +` same | -- | end of the kernel reserve sweep — runtime ceiling, equals `0x2B000 + 20 B` on `-m 1` and `0x2B000 + 32 KB` on `-m 1024`; everything past this in conventional RAM is owned by the bitmap allocator (subject to E820's reserved regions, including the VGA aperture at `0xA0000..0xFFFFF`) | -- |
...
| `0xBFFE0000..0xBFFEFFFF` | 64 KB | unmapped (stack guard region) |
| `0xBFFF0000..0xBFFFFFFF` | 64 KB | user stack (16 pages, top at `USER_STACK_TOP`) |
| `0xC0000000` | -- | `USER_STACK_TOP` (one past end of stack; equals user/kernel boundary) |
| `0xC0000000..` | 1 GB | kernel half (PDEs 768..1023, copy-imaged from `kernel_idle_pd`) |
...
- `src/memory_management/frame.asm` — Bitmap physical-frame allocator: `frame_alloc` / `frame_free` / `frame_init` (two-pass E820 walker — first pass finds the highest type=1 frame base, between passes sizes the bitmap, second pass marks free regions) / `frame_reserve_range`.  Bitmap is sized at boot to fit installed RAM up to a 1 GB ceiling (LAST_KERNEL_PDE in kernel.asm) and lives in the post-kernel cluster at FRAME_BITMAP_PHYS.
...
- `src/c/` programs written in the C subset: `arp`, `asm`, `asmesc`, `bigbss`, `bits`, `booltest`, `cat`, `cftest`, `chmod`, `cp`, `date`, `dns`, `draw`, `echo`, `edit`, `fctest`, `gdemo`, `gptest`, `gtable`, `hello`, `inctest`, `loop`, `loop_array`, `ls`, `mkdir`, `mv`, `netinit`, `netrecv`, `netsend`, `nullderef`, `okptest`, `ping`, `pintest`, `rm`, `rmdir`, `shell`, `stackbomb`, `uptime`.  Smoke tests in this list cover specific cc.py / kernel paths: `asmesc` (file-scope + statement-form `asm(...)` inline-asm escape), `bigbss` (256 KB BSS allocation across per-program PD pages), `bits` (bitwise operators `|` `^` `~` `<<` `>>` `&` and compound-assignment forms), `booltest` (booleanized comparison BinOps used as values), `cftest` / `fctest` (call-flow / function-call edge cases), `gdemo` / `gtable` (file-scope globals), `gptest` (user-fault `#GP` kill path in `idt.asm`'s `exc_common` — executes `cli` at CPL=3, expects shell respawn), `inctest` (`#include` directive, pairs with `src/c/inctest.h`), `nullderef` (`#PF` on virt 0 with the unmapped PTE[0] guard), `okptest` (user-buffer pointer validation via `access_ok`), `pintest` (cc.py register pin / spill paths), `stackbomb` (16-page user-stack overflow into the unmapped guard region).
