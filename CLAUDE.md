...
- `src/c/` programs written in the C subset: `arp`, `cat`, `chmod`, `cp`, `date`, `dns`, `draw`, `echo`, `edit`, `gdemo`, `gtable`, `hello`, `loop`, `loop_array`, `ls`, `mkdir`, `mv`, `netinit`, `netrecv`, `netsend`, `ping`, `shell`, `uptime`. `gdemo` and `gtable` are smoke tests for cc.py's file-scope globals.
