...
- **Disk buffers** (`sector_buffer`, 512 B; `ext2_sd_buffer`, 1 KB) live in the post-kernel reserved region right after the NIC TX buffer.  `bbfs.asm` and `ext2.asm` reach them through 32-bit `mov ebx, sector_buffer` style addressing — the legacy 16-bit `[bx+offset]` constraint is gone.
...
- **Kernel stack** at phys `KERNEL_RESERVED_BASE..KERNEL_RESERVED_BASE+0x2000` (8 KB; currently ~`0x2A000..0x2C000`, shifts with `kernel.bin` size).  `KERNEL_RESERVED_BASE = page_align(0x20000 + sizeof(kernel.bin))` is computed by `make_os.sh` and passed via `-DKERNEL_RESERVED_BASE=N` to the second `kernel.asm` pass and to `boot.asm`.  Lives outside `kernel.bin` to avoid 8 KB of zero padding on disk; reachable immediately after paging because PDE[768]'s direct map covers phys `0..0x3FFFFF`; reserved via `frame_reserve_range` at boot.  `kernel_stack` / `kernel_stack_top` are `equ`s in `kernel.asm`.
...
| ~`0x2CE00..0x2D1FF` | `0xC002CE00..` | 1024 B | `ext2_sd_buffer` (sliding 2-sector window for `ext2_search_blk`) | no |
| ~`0x2E000..0x35FFF` | `0xC002E000..` | 32 KB | `program_scratch` (vfs_load staging buffer; page-aligned above the disk-buffer cluster) | no |
| ~`0x36000..0x36FFF` | `0xC0036000..` | 4 KB | boot PD (`BOOT_PD_PHYS`; promoted to `kernel_pd_template`) | no |
| ~`0x37000..0x37FFF` | `0xC0037000..` | 4 KB | first kernel PT (`FIRST_KERNEL_PT_PHYS`) | no |
| ~`0x38000+` | `0xC0038000+` | -- | `LOW_RESERVE_BYTES` ceiling — the kernel reserves only this region (plus the vDSO target page at `0x10000`); everything else in conventional RAM is owned by the bitmap allocator (subject to E820's reserved regions, including the VGA aperture at `0xA0000..0xFFFFF`) | -- |
