...
- **Resident kernel** (`kernel.bin`) is loaded at physical `0x20000` and runs at virtual `0xC0020000`.  The 256 MB direct map at `0xC0000000..0xCFFFFFFF` mirrors physical `0..256 MB` so the kernel can reach any frame the bitmap allocator returns.
...
| ~`0x2B000..0x2CFFF` | `0xC002B000..` | 8 KB | `frame_bitmap` (256 MB ceiling; `frame_init` zero-fills before any allocator call, so the bytes don't ride on disk inside `kernel.bin`) | no |
| ~`0x2D000+` | `0xC002D000+` | -- | `LOW_RESERVE_BYTES` ceiling — the kernel reserves only this region (plus the vDSO target page at `0x10000`); everything else in conventional RAM is owned by the bitmap allocator (subject to E820's reserved regions, including the VGA aperture at `0xA0000..0xFFFFF`) | -- |
