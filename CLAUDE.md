...
- `src/asm/` single-purpose utilities (behavior follows the name): `arp`, `netrecv`, `netsend`.
- `src/c/` programs written in the C subset: `cat`, `chmod`, `cp`, `date`, `draw`, `echo`, `hello`, `loop`, `loop_array`, `ls`, `mkdir`, `mv`, `netinit`, `uptime`.
